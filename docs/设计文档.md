...
当前自动化结果为：19 项网关测试通过，其中新增一次性配对码轮换、凭据精确过期、已建立 WSS 到期关闭和协议 `expiresAt` 校验；合法 CA 的 HTTPS/WSS 端到端链路与安全负向用例通过；既有 1000 条命令成功率 100%，命令 P95 4.551 ms，状态事件 P95 4.861 ms；30 秒稳定性冒烟内 RSS 增长 0.301 MB。API 24 模拟器已完成 HTTPS 健康检查、网关配对、HUKS 保存、APP 重启恢复和四设备快照加载。HUKS 篡改拒绝的设备执行、四业务页面完整回归、30 分钟稳定性、Native 控制与网关端侧互通及 APP 前后台恢复仍需补证。
