...
| 编号 | 功能域 | 设计要求 | 验收证据 |
| --- | --- | --- | --- |
| F-CC-01 | 控制中心 | 完成网关配对、连接状态展示；断线后返回配对页并记住上次地址 | 配对记录、连接状态、断线与地址记忆测试 |
...
| NF-REL-01 | 可靠性 | 断线后立即停止控制并返回配对页；重新配对后完成全量状态同步 |
...
- APP 使用 HUKS 本地 AES-256-GCM 包装密钥加密配对凭据、会话数据密钥、有效期和网关地址，将版本号、96 位随机 IV、密文与 128 位认证标签保存在 Preferences；解密时认证标签通过 `HUKS_TAG_AE_TAG` 独立传入，解密材料仅在进程内短暂使用。
...
stateDiagram-v2
    ...
    state "在线" as Online
    state "安全配对" as Pairing
    state "已关闭" as Closed

    Disconnected --> Connecting: 用户连接/应用启动
    Connecting --> Authenticating: TLS/WSS 握手成功
    Connecting --> Pairing: 网络错误或超时
    Authenticating --> Online: 凭据有效
    Authenticating --> Pairing: 认证失败/凭据过期
    Online --> Pairing: 连接断开/心跳超时
    Pairing --> Connecting: 用户输入当前一次性配对码
    Online --> Closed: 用户主动断开
    Pairing --> Closed: 用户主动断开
    Closed --> Pairing: 用户重新连接
```

WebSocket 断开或心跳超时后，APP 立即禁用控制并返回安全配对页。`PairingPreferenceStore` 只记住上次网关地址，不保存配对码；用户输入网关当前一次性配对码后重新建立会话并获取全量设备快照。应用进入前台时必须检查凭据有效期和连接状态；受系统后台策略限制时，不承诺 APP 长时间在后台维持连接。
...
- 网关断线时保留最近一次缓存状态，但以“离线快照”标识并禁用控制按钮；页面返回安全配对页并预填上次网关地址。

网关每 15 秒发送心跳或状态事件。连续 45 秒未收到任何有效消息时，APP 将连接标记为离线并返回安全配对页。

### 5.1.1 首页导航、总控与设备页复用（2026-07-19）

- `Index` 底部固定提供“家 + 智能 + 设置”三个主导航；`ControlCenterTab` 在家页面内根据可展示设备的房间集合生成“全屋 + 动态房间”横向分段。
- `ControlCenterTab` 是全屋入口，按 `DeviceRegistry` 中的功能定义汇总灯光、窗帘、风扇、环境和门锁。门锁标记为 `homeOnly`，因此不会出现在房间筛选结果中。
- 主页按钮把 `selectedRoom` 和房间分段滚动位置同时复位，避免只切换数据而留下错位的选中上下文。
- 房间分段只渲染当前房间的可展示设备；设备卡统一进入 `DeviceDetail`。
- `FeatureDevices` 与 `DeviceDetail` 不再实现另一套简化控制逻辑，而是按类型复用既有 `LightingTab`、`TemperatureTab`、`DoorLockTab`，通过 `deviceId` 和 `showTitle` 参数保证从总控、房间、详情页进入后的状态、开关、确认弹窗和日志行为一致。
- 窗帘与风扇由通用能力描述驱动分类、状态摘要和控件，不增加 APP 专用类型分支。
- 门锁卡片与智能场景统一使用 `LockIcon` 和开/关锁 SVG，避免不同页面使用含义不一致的字符图标。
...
| --- | --- | --- | --- |
| `HaierSimAdapter` | 开关、模式、目标温度、风速 | `HAIER_SIM\|POWER=ON\|MODE=COOL\|TEMP=24\|FAN=HIGH` | 自定义演示格式，不代表海尔真实协议 |
| `GreeSimAdapter` | 开关、模式、目标温度、风速 | `GREE_SIM\|PWR:1\|MODE:COOL\|T:24\|FAN:HIGH` | 自定义演示格式，不代表格力真实协议 |
| `MideaSimAdapter` | 开关、模式、目标温度、风速 | `MIDEA_SIM\|ON;COOL;24;HIGH` | 自定义演示格式，不代表美的真实协议 |
...
| 回家模式 | 用户主动执行 | 调整两盏客厅灯、恢复客厅空调自动模式并锁门 |
| 清凉/温暖/除湿模板 | 用户创建或运行一次 | 批量设置全部空调的开关、模式、温度和风速 |
| 自定义常用场景 | 用户主动执行 | 灯光、空调及通用能力设备执行保持、开启/打开或关闭；门锁只允许保持或上锁 |

`SmartRuleStore` 使用 Preferences 保存模板规则、自定义规则、内置场景覆盖和自定义常用场景，并兼容旧格式迁移。保存数据只描述用户配置，不作为设备权威状态；实际执行仍通过 `DeviceManager`、加密传输和网关状态版本链路。通用设备场景优先映射主开关，其次映射 `open/close` 按钮或主滑块；门锁只暴露保持/上锁，解锁仍必须走二次确认。当前只有网关自动照明在 APP 断线后持续运行，其他模板在创建、点击场景或“运行一次”时下发。
...
├── 底部导航：家 / 智能 / 设置
├── 家：全屋 / 动态房间分段
│   ├── 全屋指标 → 功能卡（灯光、窗帘、风扇、环境、门锁）
│   └── 功能页 / 设备详情 → 类型控制模块（按 deviceId 复用）
│   └── 房间设备列表（不含门锁）→ 设备详情
├── 智能
│   ├── 已保存规则：启停 / 详情 / 运行一次 / 多选删除
│   ├── 推荐模板：感应照明 / 清凉 / 温暖 / 离家 / 除湿
│   └── 常用场景：回家 / 离家 / 自定义编辑
└── 设置与诊断
...
| 方法与路径 | 鉴权 | 用途 | 主要返回 |
| --- | --- | --- | --- |
| `POST /api/v1/pair` | 一次性配对码 | 注册客户端并建立后续凭据 | 客户端编号、凭据材料、`issuedAt`、`expiresAt` |
...
- 建立连接或重新配对后，APP 调用 `GET /api/v1/devices` 覆盖本地设备缓存，再开始处理增量事件。
...
| 数据 | 存储方式 | 安全要求 |
| --- | --- | --- |
| 配对令牌、数据密钥、有效期、网关地址 | HUKS + Preferences | HUKS AES-256-GCM 包装；Preferences 仅保存版本、IV、密文与认证标签 |
| 上次网关地址 | Preferences | 仅用于配对页预填；不保存配对码、凭据或数据密钥 |
| 当前设备快照 | 进程内存 | 重新配对后以网关全量快照覆盖，不跨 APP 重启保存 |
...
1. 网关生成 6 位一次性配对码，有效期 5 分钟；配对成功或到期后立即轮换为不同的新码。
2. APP 在已验证的 HTTPS 链路上提交配对码和随机客户端编号。
3. 网关验证成功后签发高熵客户端凭据和数据密钥材料，并返回 `issuedAt`、`expiresAt`；凭据只在配对响应中返回一次，默认有效期 24 小时且可配置。
4. APP 立即使用 HUKS AES-256-GCM 包装凭据、数据密钥、有效期和网关地址，Preferences 只保存密文封装。
5. 网关在进程内保存令牌摘要和会话数据密钥，不保存明文令牌，也不把会话材料写入 SQLite 或日志。
6. WSS 握手携带客户端凭据，网关认证通过后才接受命令或订阅事件；网关在每条命令和空闲等待期间复查有效期，到期即以 4401 关闭连接。
...
- HUKS 包装操作使用 AES-256-GCM、96 位随机 IV、128 位认证标签和固定版本化 AAD；解密时以 `HUKS_TAG_AE_TAG` 传入认证标签。首次运行时把“密钥不存在”作为正常分支创建密钥，其他 HUKS 错误不得降级为明文保存。
...
2. 准备证书和网关配置，启动虚拟网关。
3. 网关通过模块化驱动初始化 13 台默认模拟设备。
4. 使用 HarmonyOS SDK API 24 构建 HAP，安装到设备或模拟器。
5. APP 输入网关 HTTPS 地址并执行健康检查。
6. 在网关生成一次性配对码，在 APP 完成配对。
7. APP 建立 WSS 长连接并获取设备全量快照。
8. 依次演示控制中心、多房间切换、智能规则与常用场景、自动照明、空调风速与品牌切换、智能门禁和离家模式。
9. 注入网关断线、门锁卡滞、错误证书或消息篡改，展示回配对页、地址记忆、错误处理与安全日志。
...
| 设备编号示例 | 名称 | 类型 | 房间 |
| --- | --- | --- | --- |
| `light-*`（5 台） | 主灯、氛围灯、顶灯、阅读灯、镜前灯 | 灯光 | 客厅、主卧、次卧、浴室 |
| `env-*`（3 台） | 环境/湿度传感器 | 环境 | 客厅、主卧、浴室 |
| `ac-*`（2 台） | 客厅空调、主卧空调 | 空调 | 客厅、主卧 |
| `door-entry-01` | 入户门锁 | 门锁 | 入户区 |
| `curtain-living-01` | 客厅智能窗帘 | 通用能力设备 | 客厅 |
| `fan-bedroom-01` | 卧室循环风扇 | 扩展驱动设备 | 次卧 |

默认快照共 13 台。7 分钟演示可优先覆盖客厅主灯、客厅环境/空调、门锁、窗帘和扩展风扇，再用房间分段及离家场景证明多设备聚合能力。
...
| FT-16 | WebSocket 断开 | 返回安全配对页，保留上次网关地址但不保存配对码 |
| FT-17 | 控制智能窗帘与扩展风扇 | 通用控件下发命令，状态版本递增，APP 无专用类型分支 |
| FT-18 | 调节空调风速 | 自动、低、中、高四档经品牌适配器编码并回传权威状态 |
| FT-19 | 保存并恢复智能规则/场景 | APP 重启后配置恢复；运行一次仍通过网关执行并展示结果 |

### 10.2 异常与可靠性测试

| 用例 | 场景 | 预期结果 |
| --- | --- | --- |
| RT-01 | 网关进程停止后恢复 | APP 返回配对页；输入新配对码后在线并同步权威快照 |
...
当前代码已实现 HUKS 密钥首次创建、AES-256-GCM 加密、Preferences 密文写入、认证标签独立传参、解密恢复和会话清理。API 24 模拟器已完成健康检查、网关配对、会话保存、APP 强制停止后重启恢复和四设备快照加载，未再出现 `12000011` 或认证标签错误；Debug HAP 构建通过。认证标签篡改拒绝的 ohosTest 已加入并完成编译，但命令行设备测试仍需要本机签名配置，HarmonyOS 6.1.1 真机验证仍未执行。
...
| 断线恢复 | 随机停止网关后恢复 | APP 返回配对页并保留地址；使用新配对码后完成快照同步 |
...
- HarmonyOS 6.1.1 / API 24：历史设备证据已覆盖 HAP 安装、配对、四设备快照和 HUKS 重启恢复；2026-07-19 合并后的 13 设备、智能页和空调风速已通过 Debug HAP 编译，完整交互仍需继续回归。
...
设备接入采用注册式 `DeviceDriver`：

1. 在 `devcontrol_gateway/extensions/` 新增驱动，声明设备类型和初始设备，实现命令校验与状态转换；需要遥测或渐进运动时实现 `tick`。
2. 注册表统一处理在线检查、状态冲突、故障注入、版本递增和公共状态事件，并拒绝重复类型或设备编号。
3. 灯光、环境、空调和门锁保留强类型模型及专用页面；其他设备使用 `category/state/controls` 通用契约。
4. APP 只渲染通过标识符、状态类型、控件数量、滑块范围和枚举选项校验的能力描述；网关驱动仍执行最终参数校验。
5. 新增常见设备只需实现驱动并在 `composition.py` 显式注册，不修改 APP 解析、功能分组、详情路由或通用控制组件。
6. 智能窗帘验证渐进位置事件；卧室虚拟风扇验证新增独立驱动后，开关、枚举和摇头控件可直接上线。

### 11.2 新空调品牌

当前新增品牌还不是单点注册：除实现并注册 `BrandAdapter` 外，还要同步协议 Schema 品牌枚举、ArkTS `DeviceBrand` 和空调页面选项。因此本轮只证明新设备类型可以快速上线，不把新空调品牌列为同等级扩展能力。若品牌接入成为高频需求，应把支持品牌及显示名称放入空调快照能力描述，并由 APP 动态生成品牌选项。真实协议必须来自厂商公开 SDK 或合法资料，且模拟与真实适配器使用不同名称。

### 11.3 新传输协议

`GatewayTransport` 将 APP 业务与具体网络实现隔离。当前网关已支持 HTTPS REST、WSS 和 HTTPS/SSE，并提供默认关闭的 MQTT 5 over TLS 外部 Broker 桥接。HTTPS、WSS 和 MQTT 命令统一进入 `GatewayService.process_command()`，因此不会出现某个新增传输绕过 AES-GCM、重放防护、状态版本或审计的情况。MQTT 启用时强制校验 Broker CA，并要求 mTLS 或账号认证；Broker 侧主题 ACL 属于部署必选项。

当前仍不宣称支持 CoAP/DTLS、Matter、Zigbee、蓝牙或厂商私有协议。若未来接入 CoAP，必须同时实现 DTLS、设备身份与资源授权，并通过真实受限终端互操作测试，不能只增加消息编解码器便标记为支持。
...
| APP 后台连接受系统策略影响 | 后台期间状态可能延迟 | 前台恢复时检查会话；连接已断则返回配对页，不承诺无限后台保活 |
...
本系统以 OpenHarmony 生态 APP 和独立虚拟网关构成真实网络拓扑，在无实体硬件条件下实现控制中心、照明中心、温湿度与空调中心、智能门禁四类核心功能。系统采用 ArkTS + Native C++ 技术路线，通过 WSS/HTTPS、AES-256-GCM 和 HUKS 分别覆盖传输层、数据层和密钥层安全，并通过设备状态版本、幂等消息、断线回配对页、地址记忆和权威快照保证状态一致性。
...
截至 2026-07-19，设计已落到以下真实模块：

| 设计项 | 实际实现 |
| --- | --- |
| 权威网关 | `VirtualGateway/devcontrol_gateway/`，FastAPI + HTTPS/WSS/SSE + 可选 MQTT 5 TLS + SQLite |
| 协议契约 | `VirtualGateway/protocol/protocol-v1.schema.json` |
...
| 会话保护 | `GatewaySessionStore.ets`，HUKS AES-256-GCM；认证标签通过 `HUKS_TAG_AE_TAG` 校验，Preferences 仅保存加密封装 |
...
| 自动化与设备 | 模块化驱动注册表、13 台默认设备、窗帘/风扇通用能力、空调四档风速、智能规则与场景持久化 |
| 维护接口 | 仅监听 `127.0.0.1:18444`，支持离线、卡锁、低电量、延迟和环境注入 |

当前自动化结果为：2026-07-19 合并回归的 30 项网关测试通过，覆盖 13 台默认快照、8 设备离家场景、4 设备回家场景、空调风速、驱动注册、智能窗帘、虚拟风扇首次接入、一次性配对码轮换、凭据精确过期、已建立 WSS 到期关闭、HTTPS 命令、SSE、MQTT 5 TLS 桥接和 TLS 1.2 下限；Debug HAP 同日构建成功。此前合法 CA 的 HTTPS/WSS 端到端链路与安全负向用例通过；2026-07-18 隔离端口回归的 1000 条命令成功率 100%，命令 P95 4.188 ms，状态事件 P95 4.538 ms；30 秒稳定性冒烟内 RSS 增长 0.301 MB。API 24 模拟器的既有证据覆盖四设备快照和 HUKS 重启恢复，合并后的 13 设备完整页面、智能页、真实 MQTT Broker、HUKS 篡改拒绝设备执行、30 分钟稳定性、Native 互通及 APP 前后台恢复仍需补证。
...
| 权威网关核心 | FastAPI HTTPS/WSS、SQLite、模块化驱动下的 13 设备状态机、场景、三品牌空调适配、配对限流和回环维护接口 | 已完成并通过自动化 |
| APP 架构重构 | 判别联合设备模型、严格解析、`GatewayTransport`、权威快照/事件仓库、待处理命令和中文错误映射 | 已完成并通过构建 |
| 安全端到端链路 | 演示 CA/SAN、带有效期 Bearer 会话、Native AES-256-GCM、随机 messageId/nonce、HUKS 会话封装、WSS 心跳与到期关闭 | 代码完成；TLS/WSS 自动化与 HUKS 模拟器保存/恢复通过；篡改设备测试待签名执行 |
| 业务页面 | 家/房间、智能规则与场景、灯光、环境/空调、门锁、通用设备、设置诊断、离线禁用与缓存状态 | 代码完成并通过构建；13 设备完整交互回归待完成 |
| 可靠性、安全与性能 | 幂等、重放、版本冲突、一次性配对码、凭据过期、错误证书、1000 命令性能、短时稳定性和敏感信息扫描 | 自动化指标通过；30 分钟和前后台/网络切换待设备验证 |
| 文档、演示与发布 | 启动/证书/测试/构建脚本、用户文档、测试报告、追踪矩阵及演示签名 HAP | 本地交付基线完成；正式发布签名和真机证据待补 |

总体上，软件架构、协议、安全边界、网关业务能力和 APP 功能代码已经达到设计目标的实现阶段，现阶段剩余工作主要是设备侧验证与证据收口，而不是继续扩展核心架构。最终验收前必须完成：HUKS 篡改拒绝设备测试、四页面交互、错误证书在 NetworkKit 中的拒绝、断线回配对页和前后台恢复、HarmonyOS 6.1.1 真机验证以及连续 30 分钟稳定性测试。
