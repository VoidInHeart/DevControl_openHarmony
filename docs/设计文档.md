...
- 支持“离家模式”：关闭全部灯光、空调、浴霸、加湿器和窗帘，并锁定门锁；环境传感器保持运行。任一步骤失败时展示逐设备结果，不把部分成功误报为全部成功。
...
- `Index` 顶部标签由“总控 + 动态房间 + 设置”构成；房间标签从可展示设备的房间集合生成。
- `ControlCenterTab` 是总控入口，按 `DeviceRegistry` 中的功能定义汇总灯光、窗帘、环境和门锁。门锁标记为 `homeOnly`，因此不会出现在房间页。
- `RoomTab` 只渲染当前房间的可展示设备；设备卡统一进入 `DeviceDetail`。
...
- 窗帘已由 `CurtainDriver` 作为预置设备接入；APP 通过统一能力描述提供打开、暂停、关闭和 0～100 开合度控制。当前仅限制二维码动态注册，不能据此误称窗帘未接入。
- 总控内容区使用剩余空间布局承接顶部标签栏，避免标签栏与“我的全屋”卡片之间出现非预期留白。
...
| 离家模式 | 用户主动执行 | 关闭全部灯光、空调、浴霸、加湿器和窗帘，锁定全部门锁；环境传感器保持运行 |
...
首页（控制中心）
├── 顶部标签：总控 / 动态房间 / 设置
├── 总控：全屋指标 → 功能卡（灯光、窗帘、环境、门锁）
│   └── 功能页 / 设备详情 → 类型控制模块（按 deviceId 复用）
├── 房间：当前房间设备列表（不含门锁）
│   └── 设备详情 → 类型控制模块（按 deviceId 复用）
└── 设置
...
- 项目 CA 的公钥信任包随 APP 资源提供；网关在本机生成私钥和 CSR，签发管理台只持有仓库外受保护的项目 CA 私钥，并在人工核对 DNS/IP SAN 后签发网关证书。
...
1. 签发管理员使用仅监听回环地址的管理台，在仓库外创建受口令保护的项目 CA 私钥，并将 CA 公钥写入 APP 原始资源。
2. 网关在本机为其局域网主机名或固定 IP 生成 P-256 私钥与 CSR；只将 CSR 交给签发管理员。
3. 管理员人工核对 DNS/IP SAN 归属后签发服务端证书，只将 `gateway.crt` 回传给网关。
4. 将网关证书和私钥放在网关受限目录；私钥、CSR 和 CA 私钥均不进入仓库和 APP。
5. 根 CA 轮换期间先发布包含旧、新 CA 的 App 信任包，网关全部换发后再移除旧根；当前未提供 CRL/OCSP 在线吊销服务。
...
| FT-13 | 执行离家模式 | 灯光、空调、浴霸、加湿器和窗帘关闭，门锁锁定，环境传感器保持运行，逐设备展示结果 |
...
- HarmonyOS 6.1.1 / API 24：HAP 构建、安装、配对、首页四设备快照和 HUKS 重启恢复已通过；四大核心功能完整交互仍需继续回归。
...
1. 驱动声明设备类型和初始设备，实现命令校验与状态转换；需要遥测或渐进运动时实现 `tick`。
...
5. 新增常见设备通常只需实现并注册驱动，不修改 APP 解析、功能分组、详情路由或通用控制组件。
6. 默认智能窗帘通过 `open/close/stop/setPosition` 和渐进位置事件验证完整扩展链路。
...
| 项目 CA 缺少在线吊销服务 | 密钥泄露后的处置时效受限 | 不关闭证书验证；使用双根轮换与短期网关证书，生产化前接入 CRL/OCSP 与受管密钥服务 |
...
截至 2026-07-26，设计已落到以下真实模块：

| 设计项 | 实际实现 |
| --- | --- |
| 权威网关 | `VirtualGateway/devcontrol_gateway/`，FastAPI + HTTPS/WSS + SQLite |
...
| 自动化与设备 | 网关内灯光、环境、空调、浴霸、加湿器、门锁、窗帘状态机与三种模拟空调品牌适配器 |
| 维护接口 | 仅监听 `127.0.0.1:18444`，支持离线、卡锁、低电量、延迟和环境注入；管理页在接口不可达时清除旧快照、禁用操作并提示离线 |

当前自动化结果为：43 项网关测试通过，覆盖驱动注册、智能窗帘、一次性配对码轮换、凭据精确过期、已建立 WSS 到期关闭、协议 `expiresAt` 校验和本机管理页离线恢复契约；合法 CA 的 HTTPS/WSS 端到端链路与安全负向用例通过；2026-07-18 隔离端口回归的 1000 条命令成功率 100%，命令 P95 4.188 ms，状态事件 P95 4.538 ms；30 秒稳定性冒烟内 RSS 增长 0.301 MB。API 24 模拟器已完成 HTTPS 健康检查、网关配对、HUKS 保存、APP 重启恢复和四设备快照加载。HUKS 篡改拒绝的设备执行、五设备页面完整回归、30 分钟稳定性、Native 控制与网关端侧互通及 APP 前后台恢复仍需补证。
...
| 权威网关核心 | FastAPI HTTPS/WSS、SQLite、五设备驱动状态机、自动化、三品牌空调适配、配对限流和回环维护接口 | 已完成并通过自动化 |
| APP 架构重构 | 判别联合设备模型、严格解析、`GatewayTransport`、权威快照/事件仓库、待处理命令和中文错误映射 | 已完成并通过构建 |
| 安全端到端链路 | 项目 CA/CSR、SAN、带有效期 Bearer 会话、Native AES-256-GCM、随机 messageId/nonce、HUKS 会话封装、WSS 心跳与到期关闭 | 代码完成；CSR/签发、TLS/WSS 自动化与 HUKS 模拟器保存/恢复通过；篡改设备测试待签名执行 |
