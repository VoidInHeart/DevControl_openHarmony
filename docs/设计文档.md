...
### 5.1.1 首页导航、总控与设备页复用（2026-07-17）
...
- 窗帘分类已预留，但当前网关未提供窗帘协议适配器，故只呈现空状态，不下发控制命令。
...
当前自动化结果为：30 项网关测试通过，覆盖驱动注册、智能窗帘、虚拟风扇首次接入、一次性配对码轮换、凭据精确过期、已建立 WSS 到期关闭、HTTPS 命令、SSE、MQTT 5 TLS 桥接和 TLS 1.2 下限；合法 CA 的 HTTPS/WSS 端到端链路与安全负向用例通过；2026-07-18 隔离端口回归的 1000 条命令成功率 100%，命令 P95 4.188 ms，状态事件 P95 4.538 ms；30 秒稳定性冒烟内 RSS 增长 0.301 MB。API 24 模拟器已完成 HTTPS 健康检查、网关配对、HUKS 保存、APP 重启恢复和四设备快照加载。真实 MQTT Broker ACL/断线联调、HUKS 篡改拒绝的设备执行、六设备页面完整回归、30 分钟稳定性、Native 控制与网关端侧互通及 APP 前后台恢复仍需补证。
...
| 权威网关核心 | FastAPI HTTPS/WSS、SQLite、六设备驱动状态机、自动化、三品牌空调适配、配对限流和回环维护接口 | 已完成并通过自动化 |
