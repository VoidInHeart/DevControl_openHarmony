...
设备二维码包含序列号、展示名称、设备类型、功能分类、房间标识、能力列表和网关签发的 ES256 静态设备身份证书（紧凑 JWS）。App 只解析声明；`POST /api/v1/devices/register` 必须由已配对的 Bearer 会话调用。网关会核对证书的签名、签发者、受众及全部设备声明，再将在线与能力校验交给设备驱动；驱动确认可接入后才写入快照。证书不带有效期或一次性 `jti`，同一设备可重复扫描，删除后也可用原二维码重新加入。

VirtualGateway 当前为 `light` + `lighting` 提供此真实演示链路，支持 `setPower`、`setBrightness`、`setAutomationConfig`。成功加入的设备带有 `removable: true`，可由 `DELETE /api/v1/devices/{deviceId}` 移除；删除后网关广播 `device.removed`，App 同步最新快照。
