...
设备二维码包含序列号、展示名称、设备类型、功能分类、能力列表和网关签发的 ES256 静态设备身份证书（紧凑 JWS），不包含房间标识。App 识别后由用户选择已有房间，并将该 `roomId` 仅随 `POST /api/v1/devices/register` 的已配对 Bearer 请求发送。网关会核对证书的签名、签发者、受众及设备固有声明，再校验所选房间存在，并将在线与能力校验交给设备驱动；驱动确认可接入后才写入快照。证书不带有效期或一次性 `jti`，同一设备可重复扫描，删除后也可用原二维码重新加入到任一可选房间。
